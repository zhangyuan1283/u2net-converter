...
# Check tf installed. This project uses 2.13.1 for compatibility reasons
...
if __name__ == "__main__":
    ...
    net.load_state_dict(torch.load('./content/u2net.pth', map_location='cpu'))
    net.eval()

    # Upload woman.png, view details and resize to model input for portrait generation(512,512)
    image = cv2.imread('./content/Athlete-Intake.jpg')
    print(image.shape)
    ...
    # This component is for portrait generation specifically
    # Normalize the input
    ...
    tmpImg = np.zeros((image_resized.shape[0],image_resized.shape[1],3))
    input = image_resized/np.max(image_resized)
    tmpImg[:,:,0] = (input[:,:,0]-0.406)
    tmpImg[:,:,1] = (input[:,:,1]-0.456)
    tmpImg[:,:,2] = (input[:,:,2]-0.485)
    ...
    pred = 1.0 - d1[:,0,:,:]
    pred = normPRED(pred)
    save_output("Athlete",pred,"/content")
    pil_im = Image.open('./content/Athlete-Intake.jpg', 'r')
    ...
    # torch.onnx.export(net, dummy_input, "u2net.onnx", input_names = ['input'], output_names = ['output'])

    # Export the model - this adds extra configurations to make sure that the model is self contained
